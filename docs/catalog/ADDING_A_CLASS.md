...
`crates/homecooked-schema/src/catalog/mod.rs` (see `wine_cooler` / `ice_maker` / `sous_vide` / `multi_cooker` / `toaster_oven` / `dehumidifier` / `range_hood` / `steam_oven` / `cooktop` / `humidifier` / `freezer` / `fridge_freezer`).
