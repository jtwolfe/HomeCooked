...
Version **0.1.0** — design extension (docs sketch); device catalog points landed for water_heater/fridge (see history).
...
| Heat port | Advertised attachment on an appliance | Optional class points (`thermal_port_*`) on `water_heater` / `fridge`; plant objects still in `homecooked-thermal` |
...
| 0.1.0+ | Catalog/sim device telemetry surface: optional `thermal_port_id` / `direction` / `media` / `max_power_w` / `attached_reservoir_id` (RW) on `water_heater` and `fridge`. Plant types remain crate-local in `homecooked-thermal`. |
