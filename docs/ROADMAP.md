...
Version **0.1.49**. Planning doc for a long flesh-out of the catalog, control
...
optional-point depth on `wine_cooler` + `ice_maker` + `sous_vide` + `multi_cooker` + `toaster_oven` + `dehumidifier` + `range_hood` + `steam_oven` + `cooktop` + `humidifier` + `freezer`; `write_denial_matrix` + `catalog_hygiene` conformance.
...
depth beyond `wine_cooler` + `ice_maker` + `sous_vide` + `multi_cooker` + `toaster_oven` + `dehumidifier` + `range_hood` + `steam_oven` + `cooktop` + `humidifier` + `freezer`; procedure⇄thermal
...
matrix + early catalog depth (`wine_cooler` / `ice_maker` / `sous_vide` / `multi_cooker` / `toaster_oven` / `dehumidifier` / `range_hood` / `steam_oven` / `cooktop` / `humidifier` / `freezer`) ≈ **~75% of the §2
...
`ClassTable.thermal_ports`, `wine_cooler` + `ice_maker` + `sous_vide` + `multi_cooker` + `toaster_oven` + `dehumidifier` + `range_hood` + `steam_oven` + `cooktop` + `humidifier` + `freezer` optional depth, heat-port
...
- **Deeper catalog optional points** — thin tables cover all 31 Tier-B ids;
  ...
  + `multi_cooker` + `toaster_oven` + `dehumidifier` + `range_hood` + `steam_oven` + `cooktop` + `humidifier` + `freezer` (alarms / sabbath / bottle_count / humidity; ice bin/filter life +
  ...
  freezer fast_freeze/door_ajar/ice_buildup/low_temp_alarm/anti_sweat/fast_freeze_remaining/frost_clean + cold-cabinet vacation/sabbath/eco/defrost/compressor/high_temp/power_fail); remaining thin
...
| `freezer` | Optional depth: fast_freeze/door_ajar/ice_buildup/low_temp_alarm/anti_sweat/fast_freeze_remaining_s/frost_clean_needed + typical cold-cabinet vacation/sabbath/eco/defrost/compressor/high_temp/power_fail (fridge_freezer keeps shared cold-cabinet only) |
...
| later | catalog optional depth | 7 — **Started** (`wine_cooler` + `ice_maker` + `sous_vide` + `multi_cooker` + `toaster_oven` + `dehumidifier` + `range_hood` + `steam_oven` + `cooktop` + `humidifier` + `freezer`; more classes open) |
