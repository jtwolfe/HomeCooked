...
`thermal_wait` / **`thermal_offer`** (soft decline + thin fallback + **plant Counter** / `accept_counter`) / **continuous `requeue_offer`**; controller-sim-over-TCP interlock smoke
...
`thermal_offer` soft decline/fallback + plant Counter `accept_counter` + continuous `requeue_offer`) +
...
- **Procedure⇄thermal depth** — thin `thermal_wait` on reservoir `temp_c` and
  ...
  auto-accept of suggested band; continuous `requeue_offer` on wait;
...
2. ~~Simulator can load and run a small library.~~ **Done** — bundled
   `kettle_heat_80` + `reheat_dominos_microwave` + `wash_then_dry` + `oven_bake_180` + `coffee_brew_espresso` + `air_fryer_cook_200` + thin `wait_dhw_reservoir` (`thermal_wait`) + `offer_fridge_dhw` / `offer_fridge_dhw_soft` / `offer_fridge_dhw_counter` (`thermal_offer` soft decline + fallback + plant Counter) + `wait_dhw_with_requeue` (continuous `requeue_offer`); wasm `run_procedure` E2E
   auto-spawns and completes device fixtures (microwave wait uses sim `elapsed_s` ticks). Dedicated thermal procedure wasm UI landed thin (0.1.102: picker badges + one-click `run_thermal_procedure` + outcome badges; Counter fixture listed in 0.1.103).
...
1. Simulator-web UX sufficient to pick a Tier-A class, inspect capabilities,
   ...
   **Done (picker slice)** — `list_appliance_classes` / `create_device` cover
   ...
   `offer_fridge_dhw_soft` / `offer_fridge_dhw_counter` / `wait_dhw_with_requeue` listed/bundled with
