...
Version **0.1.68**. Planning doc for a long flesh-out of the catalog, control
...
`humidifier` + `freezer` + `fridge_freezer` + `beverage_cooler` + `kegerator` + `warming_drawer` + `pizza_oven` + `electric_grill` + `electric_smoker` + `espresso_machine` + `drip_coffee_maker` + `coffee_grinder` + `water_dispenser` + `toaster` + `blender` + `food_processor` + `stand_mixer` + `juicer` + `rice_cooker` + `slow_cooker` (29 classes; not catalog-complete);
...
complete** — deepen series (#56–#73 + follow-on) landed optional-point passes on **29** classes
(mostly Tier-A + Tier-B `humidifier` / `beverage_cooler` / `kegerator` / `warming_drawer` / `pizza_oven` / `electric_grill` / `electric_smoker` / `espresso_machine` / `drip_coffee_maker` / `coffee_grinder` / `water_dispenser` / `toaster` / `blender` / `food_processor` / `stand_mixer` / `juicer` / `rice_cooker` / `slow_cooker`); **13 of 31 Tier-B** ids remain thin tables
...
`freezer` / `fridge_freezer` / `beverage_cooler` / `kegerator` / `warming_drawer` / `pizza_oven` / `electric_grill` / `electric_smoker` / `espresso_machine` / `drip_coffee_maker` / `coffee_grinder` / `water_dispenser` / `toaster` / `blender` / `food_processor` / `stand_mixer` / `juicer` / `rice_cooker` / `slow_cooker` ≈ **~75% of the §2 in-scope bar, met in spirit** for
...
#66–#73** + `beverage_cooler` / `kegerator` / `warming_drawer` / `pizza_oven` / `electric_grill` / `electric_smoker` / `espresso_machine` / `drip_coffee_maker` / `coffee_grinder` / `water_dispenser` / `toaster` / `blender` / `food_processor` / `stand_mixer` / `juicer` / `rice_cooker` / `slow_cooker` (the twenty-nine classes above). Calling the target **substantially
...
CottonOptions/DryOptions over wire, remaining thin Tier-B / undepened class depth).
...
- **Deeper catalog optional points** — thin tables cover all 31 Tier-B ids;
  optional-point depth landed (PRs **#56–#57, #63–#64, #66–#73** + follow-on) on **29** classes —
  Tier-A `wine_cooler` + `ice_maker` + `sous_vide` + `multi_cooker` + `toaster_oven`
  + `dehumidifier` + `range_hood` + `steam_oven` + `cooktop` + `freezer` +
  `fridge_freezer`, plus Tier-B `humidifier` + `beverage_cooler` + `kegerator` + `warming_drawer` + `pizza_oven` + `electric_grill` + `electric_smoker` + `espresso_machine` + `drip_coffee_maker` + `coffee_grinder` + `water_dispenser` + `toaster` + `blender` + `food_processor` + `stand_mixer` + `juicer` + `rice_cooker` + `slow_cooker` (alarms / sabbath / bottle_count /
  ...
  **Remaining thin Tier-B (13):**
  `bread_maker`, `dehydrator`, `vacuum_sealer`, `ice_cream_maker`, `yogurt_maker`,
  `waffle_maker`, `pasta_maker`, `steam_cooker`, `garbage_disposal`,
  `trash_compactor`, `boiler`, `water_softener`, `water_filter`. Undepened Tier-A
  without a deepen-series optional-point pass (still tabled / thermal / procedure
  only as before): e.g. `washer`, `dryer`, `washer_dryer`, `fridge`, `dishwasher`,
  `microwave`, `oven`, `range`, `induction_hob`, `air_fryer`, `kettle`,
  `coffee_machine`, `water_heater`, `hvac`.
...
`warming_drawer`, `pizza_oven`, `electric_grill`, `electric_smoker`, `espresso_machine`, `drip_coffee_maker`, `coffee_grinder`, `water_dispenser`, `toaster`, `blender`, `food_processor`, `stand_mixer`, `juicer`, `rice_cooker`, and `slow_cooker` received deepen-series optional-point passes. The other **13** Tier-B ids below remain thin
...
Count: **31** Tier-B ids, all with thin static tables + sim; **18** optional-depth
passes (`humidifier` + `beverage_cooler` + `kegerator` + `warming_drawer` + `pizza_oven` + `electric_grill` + `electric_smoker` + `espresso_machine` + `drip_coffee_maker` + `coffee_grinder` + `water_dispenser` + `toaster` + `blender` + `food_processor` + `stand_mixer` + `juicer` + `rice_cooker` + `slow_cooker`); **13** still thin.
