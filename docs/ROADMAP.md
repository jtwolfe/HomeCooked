...
Version **0.1.47**. Planning doc for a long flesh-out of the catalog, control
...
optional-point depth on `wine_cooler` + `ice_maker` + `sous_vide` + `multi_cooker` + `toaster_oven` + `dehumidifier` + `range_hood` + `steam_oven` + `cooktop`; `write_denial_matrix` + `catalog_hygiene` conformance.
...
depth beyond `wine_cooler` + `ice_maker` + `sous_vide` + `multi_cooker` + `toaster_oven` + `dehumidifier` + `range_hood` + `steam_oven` + `cooktop`; procedure⇄thermal
...
matrix + early catalog depth (`wine_cooler` / `ice_maker` / `sous_vide` / `multi_cooker` / `toaster_oven` / `dehumidifier` / `range_hood` / `steam_oven` / `cooktop`) ≈ **~75% of the §2
...
`ClassTable.thermal_ports`, `wine_cooler` + `ice_maker` + `sous_vide` + `multi_cooker` + `toaster_oven` + `dehumidifier` + `range_hood` + `steam_oven` + `cooktop` optional depth, heat-port
...
- **Deeper catalog optional points** — thin tables cover all 31 Tier-B ids;
  ...
  + `multi_cooker` + `toaster_oven` + `dehumidifier` + `range_hood` + `steam_oven` + `cooktop` (alarms / sabbath / bottle_count / humidity; ice bin/filter life +
  ...
  cooktop keep_warm/hotspot/timer_active/paused/surface_c/element_fault/pan_detect/flame_on + boost/timer/bridge/gas faults/power_limit); remaining thin
...
| later | catalog optional depth | 7 — **Started** (`wine_cooler` + `ice_maker` + `sous_vide` + `multi_cooker` + `toaster_oven` + `dehumidifier` + `range_hood` + `steam_oven` + `cooktop`; more classes open) |
