...
mod tests {
    ...
    #[test]
    fn slow_cooker_optional_depth_points_read_and_write() {
        let mut sim = Simulator::new();
        let slow = sim.spawn(ApplianceClassId::SlowCooker).unwrap();

        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.heat_level")
                .unwrap(),
            Value::Enum("low".into())
        );
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.cook_s").unwrap(),
            Value::DurationS(600)
        );
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.pot_present")
                .unwrap(),
            Value::Bool(true)
        );
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.keep_warm")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.sabbath_mode")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.eco_mode").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.heater_on")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.high_temp_alarm")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.lid_open").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.timer_s").unwrap(),
            Value::DurationS(0)
        );
        ...
        sim.write(&slow, "class.slow_cooker.cook_s", Value::DurationS(28800))
            .unwrap();
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.cook_s").unwrap(),
            Value::DurationS(28800)
        );
        sim.write(&slow, "class.slow_cooker.keep_warm", Value::Bool(true))
            .unwrap();
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.keep_warm")
                .unwrap(),
        ...
        sim.write(&slow, "class.slow_cooker.sabbath_mode", Value::Bool(true))
            .unwrap();
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.sabbath_mode")
                .unwrap(),
        ...
        sim.write(&slow, "class.slow_cooker.eco_mode", Value::Bool(true))
            .unwrap();
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.eco_mode").unwrap(),
            Value::Bool(true)
        );
        sim.write(&slow, "class.slow_cooker.timer_s", Value::DurationS(1800))
            .unwrap();
        assert_eq!(
            sim.read_value(&slow, "class.slow_cooker.timer_s").unwrap(),
            Value::DurationS(1800)
        ...
        let err = sim
            .write(&slow, "class.slow_cooker.pot_present", Value::Bool(false))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&slow, "class.slow_cooker.heater_on", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(
        ...
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&slow, "class.slow_cooker.lid_open", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
    }
    ...
    #[test]
    fn washer_optional_depth_points_read_and_write() {
        let mut sim = Simulator::new();
        let washer = sim.spawn(ApplianceClassId::Washer).unwrap();

        assert_eq!(
            sim.read_value(&washer, "class.washer.detergent_level_percent")
                .unwrap(),
            Value::Percent(80.0)
        );
        assert_eq!(
            sim.read_value(&washer, "class.washer.unbalance").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&washer, "class.washer.sabbath_mode")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&washer, "class.washer.eco_mode").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&washer, "class.washer.door_ajar").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&washer, "class.washer.door_locked").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&washer, "class.washer.water_temp_alarm")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&washer, "class.washer.overflow_alarm")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&washer, "class.washer.detergent_low")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&washer, "class.washer.timer_s").unwrap(),
            Value::DurationS(0)
        );
        assert_eq!(
            sim.read_value(&washer, "trait.time_schedule.delay_start_s")
                .unwrap(),
            Value::DurationS(0)
        );

        sim.write(&washer, "class.washer.sabbath_mode", Value::Bool(true))
            .unwrap();
        assert_eq!(
            sim.read_value(&washer, "class.washer.sabbath_mode")
                .unwrap(),
        ...
        sim.write(&washer, "class.washer.eco_mode", Value::Bool(true))
            .unwrap();
        assert_eq!(
            sim.read_value(&washer, "class.washer.eco_mode").unwrap(),
            Value::Bool(true)
        );
        sim.write(&washer, "class.washer.timer_s", Value::DurationS(7200))
            .unwrap();
        assert_eq!(
            sim.read_value(&washer, "class.washer.timer_s").unwrap(),
        ...
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&washer, "class.washer.unbalance", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&washer, "class.washer.door_ajar", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&washer, "class.washer.door_locked", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&washer, "class.washer.water_temp_alarm", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&washer, "class.washer.overflow_alarm", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&washer, "class.washer.detergent_low", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
    }

    #[test]
    fn dryer_optional_depth_points_read_and_write() {
        let mut sim = Simulator::new();
        let dryer = sim.spawn(ApplianceClassId::Dryer).unwrap();

        assert_eq!(
            sim.read_value(&dryer, "class.dryer.anti_crease").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.dryness_percent")
                .unwrap(),
            Value::Percent(100.0)
        );
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.vent_blocked").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.drain_tank").unwrap(),
            Value::Enum("ok".into())
        );
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.sabbath_mode").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.eco_mode").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.door_ajar").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.door_locked").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.high_temp_alarm")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.lint_full").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.timer_s").unwrap(),
            Value::DurationS(0)
        );
        assert_eq!(
            sim.read_value(&dryer, "trait.time_schedule.delay_start_s")
                .unwrap(),
            Value::DurationS(0)
        );

        sim.write(&dryer, "class.dryer.sabbath_mode", Value::Bool(true))
            .unwrap();
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.sabbath_mode").unwrap(),
            Value::Bool(true)
        );
        sim.write(&dryer, "class.dryer.eco_mode", Value::Bool(true))
            .unwrap();
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.eco_mode").unwrap(),
            Value::Bool(true)
        );
        sim.write(&dryer, "class.dryer.anti_crease", Value::Bool(true))
            .unwrap();
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.anti_crease").unwrap(),
            Value::Bool(true)
        );
        sim.write(&dryer, "class.dryer.timer_s", Value::DurationS(7200))
            .unwrap();
        assert_eq!(
            sim.read_value(&dryer, "class.dryer.timer_s").unwrap(),
            Value::DurationS(7200)
        ...
        let err = sim
            .write(&dryer, "class.dryer.dryness_percent", Value::Percent(50.0))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&dryer, "class.dryer.vent_blocked", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&dryer, "class.dryer.drain_tank", Value::Enum("full".into()))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&dryer, "class.dryer.door_ajar", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&dryer, "class.dryer.door_locked", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&dryer, "class.dryer.high_temp_alarm", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&dryer, "class.dryer.lint_full", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
    }

    #[test]
    fn washer_dryer_optional_depth_points_read_and_write() {
        let mut sim = Simulator::new();
        let wd = sim.spawn(ApplianceClassId::WasherDryer).unwrap();

        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.detergent_level_percent")
                .unwrap(),
            Value::Percent(80.0)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.unbalance").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.sabbath_mode")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.eco_mode").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.door_ajar").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.door_locked")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.water_temp_alarm")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.overflow_alarm")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.detergent_low")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.timer_s").unwrap(),
            Value::DurationS(0)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.anti_crease")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.dryness_percent")
                .unwrap(),
            Value::Percent(100.0)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.vent_blocked")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.drain_tank")
                .unwrap(),
            Value::Enum("ok".into())
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.high_temp_alarm")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.lint_full").unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.dry_after_wash")
                .unwrap(),
            Value::Bool(false)
        );
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.max_dry_s").unwrap(),
            Value::DurationS(0)
        );
        assert_eq!(
            sim.read_value(&wd, "trait.time_schedule.delay_start_s")
                .unwrap(),
            Value::DurationS(0)
        );

        sim.write(&wd, "class.washer_dryer.sabbath_mode", Value::Bool(true))
            .unwrap();
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.sabbath_mode")
                .unwrap(),
        ...
        sim.write(&wd, "class.washer_dryer.eco_mode", Value::Bool(true))
            .unwrap();
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.eco_mode").unwrap(),
            Value::Bool(true)
        );
        sim.write(&wd, "class.washer_dryer.anti_crease", Value::Bool(true))
            .unwrap();
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.anti_crease")
                .unwrap(),
        ...
        sim.write(&wd, "class.washer_dryer.timer_s", Value::DurationS(7200))
            .unwrap();
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.timer_s").unwrap(),
            Value::DurationS(7200)
        );
        sim.write(&wd, "class.washer_dryer.dry_after_wash", Value::Bool(true))
            .unwrap();
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.dry_after_wash")
                .unwrap(),
        ...
        sim.write(&wd, "class.washer_dryer.max_dry_s", Value::DurationS(5400))
            .unwrap();
        assert_eq!(
            sim.read_value(&wd, "class.washer_dryer.max_dry_s").unwrap(),
            Value::DurationS(5400)
        );
        sim.write(
            &wd,
            "trait.time_schedule.delay_start_s",
        ...
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&wd, "class.washer_dryer.unbalance", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(
        ...
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&wd, "class.washer_dryer.vent_blocked", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(
        ...
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&wd, "class.washer_dryer.door_ajar", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&wd, "class.washer_dryer.door_locked", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(
        ...
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&wd, "class.washer_dryer.overflow_alarm", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&wd, "class.washer_dryer.detergent_low", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&wd, "class.washer_dryer.high_temp_alarm", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
        let err = sim
            .write(&wd, "class.washer_dryer.lint_full", Value::Bool(true))
            .unwrap_err();
        assert_eq!(err.code, ErrorCode::NotWritable);
    }
