//! Matter endpoint + cluster + attribute → HomeCooked point mapping table.
...
use crate::matter::store::MatterAttrValue;
use crate::yaml_json::yaml_to_json;

/// Example `kettle` map: OnOff + TemperatureMeasurement-style attributes.
pub const KETTLE_MATTER_MAP_YAML: &str = include_str!("../../examples/kettle_matter_map.yaml");

/// How the mock fabric stores one attribute.
...
/// One Matter attribute mapped to a catalog point.
...
impl MatterEntry {
    ...
    pub fn encode_value(&self, value: &Value) -> Result<MatterAttrValue, Error> {
    ...
    pub fn decode_attr(&self, attr: MatterAttrValue) -> Result<Value, Error> {
        match (self.value_type, attr) {
            (AttrValueType::Bool, MatterAttrValue::Bool(on)) => self.decode_bool(on),
            (AttrValueType::Int16, MatterAttrValue::Int16(raw)) => {
                self.decode_numeric(f64::from(raw))
            }
            (AttrValueType::UInt16, MatterAttrValue::UInt16(raw)) => {
                self.decode_numeric(f64::from(raw))
            }
    ...
    pub fn encode_raw(&self, raw: MatterRaw) -> Result<MatterAttrValue, Error> {
        match (self.value_type, raw) {
            (AttrValueType::Bool, MatterRaw::Bool(v)) => Ok(MatterAttrValue::Bool(v)),
            (AttrValueType::Int16, MatterRaw::Int16(v)) => Ok(MatterAttrValue::Int16(v)),
            (AttrValueType::UInt16, MatterRaw::UInt16(v)) => Ok(MatterAttrValue::UInt16(v)),
    ...
    pub fn seed_attr(&self) -> Result<MatterAttrValue, Error> {
        match self.value_type {
            AttrValueType::Bool => Ok(MatterAttrValue::Bool(self.initial_bool.unwrap_or(false))),
            AttrValueType::Int16 => {
                ...
                Ok(MatterAttrValue::Int16(n as i16))
            }
            AttrValueType::UInt16 => {
                ...
                Ok(MatterAttrValue::UInt16(n as u16))
            }
        }
    }

    fn encode_bool(&self, value: &Value) -> Result<MatterAttrValue, Error> {
        if let (Some(on_tok), Some(off_tok)) = (&self.true_token, &self.false_token) {
            ...
            return Ok(MatterAttrValue::Bool(on));
        }
        match value {
            Value::Bool(on) => Ok(MatterAttrValue::Bool(*on)),
    ...
    fn encode_numeric(&self, value: &Value) -> Result<MatterAttrValue, Error> {
        ...
        match self.value_type {
            AttrValueType::Int16 => {
                ...
                Ok(MatterAttrValue::Int16(raw_f as i16))
            }
            AttrValueType::UInt16 => {
                ...
                Ok(MatterAttrValue::UInt16(raw_f as u16))
            }
...
/// Loadable Matter ↔ HomeCooked mapping table.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
impl MatterMap {
    ...
    pub fn kettle_example() -> Result<Self, Error> {
        Self::from_yaml_str(KETTLE_MATTER_MAP_YAML)
    }

    pub fn entry_for_point(&self, point_id: &str) -> Option<&MatterEntry> {
    ...
    pub fn validate(&self) -> Result<(), Error> {
        ...
        for entry in &self.entries {
            ...
            if !attrs.insert(key) {
                return Err(Error::DuplicateMatterAttribute {
...
mod tests {
    ...
    fn hundredths_scale_encode_decode() {
        ...
        assert_eq!(attr, MatterAttrValue::Int16(8000));
...
entries:
  - point: trait.power.on
    ...
    value_type: bool
"#;
        let err = MatterMap::from_yaml_str(bad).unwrap_err();
...
entries:
  ...
  - point: trait.temperature.current_c
    ...
    scale: 0.01
"#;
        let err = MatterMap::from_yaml_str(dup).unwrap_err();
        assert!(matches!(
            err,
            Error::DuplicateMatterAttribute {
                endpoint: 1,
                cluster_id: 0x201,
                attribute_id: 0x12
            }
